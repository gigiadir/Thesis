# Shared helpers for all patient split preprocess methods and QC.
# Sourced by RankGenes, Residual, ExpressionQuantile, PatientZScore, pseudobulk build,
# compareExpressionSplitEquivalence.R, analyzeGeneSplitJaccard.R, and preprocessing_checks.

source_scdiffcom_gene_panel <- function() {
  if (exists("SCDIFFCOM_GENE_PANEL", inherits = TRUE)) {
    return(invisible(NULL))
  }
  pre_dir <- if (exists("PREPROCESS_DIR", inherits = TRUE)) {
    get("PREPROCESS_DIR", inherits = TRUE)
  } else {
  ...
  source(file.path(pre_dir, "scDiffComGenePanel.R"), local = FALSE)
}

source_scdiffcom_gene_panel()
